...
if __name__ == '__main__':
    ...
    db_storage.save_state_database(state=animal_states)

    # animal states load into database
    instruction_query_template_states = State.load_state('../dataset/animallm/query_template_state.json')
    db_storage.save_state_database(state=instruction_query_template_states)
    ...
    for state in states:
        ...
        fetched_state = db_storage.load_state_database(state_id=state_id)
